...
function renderClueList(game, clues) {
        ...
        <div class="ww-list-row">
          ...
          <div class="ww-list-meta">
            <div class="ww-list-copy">
              ...
              <div class="ww-list-title">${escapeHtml(
                clueDisplayName(clueId, record)
              )}</div>
            </div>
          ...
          <aside class="ww-list-sidebar">
            <div class="ww-list-sidebar-inner">
              ...
              <nav class="ww-list-side-nav" aria-label="Clue list navigation">
                ...
                <button type="button" class="ww-list-side-link" data-go-answers>ANSWERS</button>
              </nav>
            </div>
          </aside>
            ...
            <div class="ww-list-scroll">
              <div class="ww-list-stack">
                <div class="ww-list-status">
                  <span>${escapeHtml(
                    `${total} Clue${total === 1 ? "" : "s"} Available`
                  )}</span>
  ...
  baseButtons.forEach((button) => {
    button.addEventListener("click", () => {
      renderBaseStation(game, clues);
    });
  });

  const answersButtons = app.querySelectorAll("[data-go-answers]");
          ...
          <aside class="ww-clue-rail">
            <div class="ww-mini-shell">
              ...
              <nav class="ww-mini-textnav" aria-label="Clue navigation">
                ...
                <button type="button" class="ww-mini-textlink" data-go-answers>ANSWERS</button>
              </nav>
            </div>
          </aside>
...
function renderBaseStation(game, clues) {
  const app = getApp();
  ensureStyles();
...
function renderLife(game, clues) {
  const app = getApp();
  ensureStyles();
...
function renderAnswers(game, answers) {
  const app = getApp();
  ensureStyles();
  ...
  const list =
    Array.isArray(answers) && answers.length
      ? `<ul>${answers
          .map((a, index) => {
            const label = a && a.answer ? String(a.answer) : "";
            return `<li>${escapeHtml(`${index + 1}. ${label}`)}</li>`;
          })
          .join("")}</ul>`
